...
const Checkout = () => {
  ...
  const handleIncrementPacks = (id: number) => {
    setItems((prev) =>
      prev.map((it) => (it.id === id ? { ...it, packs: it.packs + 1 } : it))
    );
  };

  const handleDecrementPacks = (id: number) => {
    setItems((prev) =>
      prev.map((it) =>
        it.id === id && it.packs > 1 ? { ...it, packs: it.packs - 1 } : it
      )
    );
  };
  ...
  const subTotal = items.reduce((sum, it) => sum + it.pizzaPrice, 0);
  ...
  return (
    <div className="sticky top-10 mt-9 max-w-[400px]">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2.5">
                    <button
                      onClick={() => handleDecrementPacks(item.id)}
                    ...
                    <button
                      onClick={() => handleIncrementPacks(item.id)}
                      className="size-5 rounded-full bg-gray-200 hover:bg-gray-300 transition-colors flex items-center justify-center"
